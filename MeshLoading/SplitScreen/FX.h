...
namespace FX
{
	...
	*/
	...
	public:
		...
		//pull out texture by address
		const Data& Get(ID3D11ShaderResourceView *p) {
			for (auto& d : mCache)
				if (p == d.pTex)
					return d;
			assert(false);
			DBOUT("Can't find texture");
			return mCache.front();
		}
	...
	class MyFX : public Singleton<MyFX>
	{
	public:
		MyFX(ID3D11Device* pd3dDevice) {
			Init(pd3dDevice);
		}
		~MyFX() {
			Release();
		...
		void Render(Model& model, ID3D11DeviceContext *pD3DContext, MaterialExt* pOverrideMat = nullptr, DirectX::SimpleMath::Matrix* pOverrideMtx=nullptr);
		...
		DirectX::SpriteBatch *mpSpriteB = nullptr;
		...
		void FlushLines(ID3D11DeviceContext *pD3DContext);
		void EnableDepthTest(ID3D11DeviceContext *pD3DContext, bool state)
		{
			if (state)
	...
	private:
		...
		ID3D11InputLayout* mpInputLayout = nullptr, *mpInputLayoutLine = nullptr;
		//a smapler to read the texture
		ID3D11SamplerState *mpSamAnisotropic = nullptr;
		//vertex and pixel shaders
		ID3D11VertexShader* mpVS = nullptr, *mpVSLine = nullptr;
		//a complicated one if it's lit, a simple one if it isn't, also a textured option now (lit and unlit)
		ID3D11PixelShader* mpPSLit = nullptr, *mpPSUnlit = nullptr, *mpPSLitTex = nullptr, *mpPSUnlitTex = nullptr, *mpPSLine = nullptr;
		...
		//useful for rendering over the top of everything
		...
		void UpdateLineBuffer(void* pMaddedData);
	};
