...
void Game::Initialise()
{
	...
	GetIAudioMgr()->GetSongMgr()->Play("Track1", true, false, &musicHdl, 0.3f);
	timer = 0;
	...
	GetEnvironmentBuilder()->Initialise();
}
...
void Game::Load()
{
	...
	GetEnvironmentBuilder()->BuildFloor();
	...
	GetEnvironmentBuilder()->BuildSkyBox();
	GetEnvironmentBuilder()->BuildSkyscraper();
	...
	mFLaz->GetPosition() = Vector3(0, -0.02, 0.3);
	mFLaz->GetScale() = Vector3(0.03, 0.03, 5);
	...
	case State::PLAYING:
		...
		else
		{
			...
			else if ((map[zIndex][xIndex].lapPositionThree && pointTwo && !pointThree))
			{
				...
				GetIAudioMgr()->GetSfxMgr()->Play("Lap", false, false, nullptr, 0.8f);
				PlayerTurret.ResetAmmo();
			}
		}
...
void Game::Render(float dTime)
{
	...
	case State::MENU:;
		...
		GetEnvironmentBuilder()->RenderSkyBox();
		PlayerDrive.Render();
	...
	case State::PLAYING:
		...
		// Render Skybox
		GetEnvironmentBuilder()->UpdateSkyBox(PlayerTurret.GetPos());
		GetEnvironmentBuilder()->RenderEnvironment();
		...
		mpFont->DrawString(mpSpriteBatch, ss.str().c_str(), Vector2(10, 20), Colours::White, 0, Vector2(0, 0), Vector2(1.f, 1.f));

		ss.str(L"");
		ss << L"FPS:" << (int)(1.f / dTime);
		ss << L"\nTime: " << (int)timer;
		ss << L"\nLaps left: " << (int)(3 - laps);
		ss << L"\nAmmo: " << (int)(PlayerTurret.GetAmmo());
		mpFont->DrawString(mpSpriteBatch, ss.str().c_str(), Vector2(10, 0), Colours::White, 0, Vector2(0, 0), Vector2(0.5f, 0.5f));


		if (PlayerDrive.isAlive() && laps >= 3)
		{
			ss.str(L"");
			ss << L"Congratulations";
			mpFont->DrawString(mpSpriteBatch, ss.str().c_str(), Vector2(sw / 3, sh / 3), Colours::White, 0, Vector2(0, 0), Vector2(0.5f, 0.5f));
		}
		if (!PlayerDrive.isAlive())
		{
			ss.str(L"");
		...
		// render skybox
		GetEnvironmentBuilder()->UpdateSkyBox(*PlayerDrive.GetPosition());
		GetEnvironmentBuilder()->RenderEnvironment();
...
void Game::LoadMap()
{
	...
	//MaterialExt *pMat = &GetEnvironmentBuilder()->getQuad()->GetMesh().GetSubMesh(0).material;
	...
	unsigned char *data = stbi_load("../bin/data/TheRockMap.jpg", &x, &y, &n, 0);
	assert(n == 3);
	...
	for (int xi = 0; xi < MAP_WIDTH; ++xi)
		for (int yi = 0; yi < MAP_WIDTH; ++yi)
		{
			...
			{
				mCar->GetPosition() = Vector3(xi*wscale + SizeOffset, 0.02, yi*wscale + SizeOffset);
			}
			else
			{
				...
				d.lapPositionOne = (data[yi*y * 3 + xi * 3] > 200) && (data[yi*y * 3 + xi * 3 + 1] > 200) && (data[yi*y * 3 + xi * 3 + 2] < 100) ? true : false;

				d.lapPositionTwo = (data[yi*y * 3 + xi * 3] < 100) && (data[yi*y * 3 + xi * 3 + 1] > 200) && (data[yi*y * 3 + xi * 3 + 2] > 200) ? true : false;

				d.lapPositionThree = (data[yi*y * 3 + xi * 3] > 200) && (data[yi*y * 3 + xi * 3 + 1] < 100) && (data[yi*y * 3 + xi * 3 + 2] > 200) ? true : false;

				d.destructable = (data[yi*y * 3 + xi * 3] < 100) && (data[yi*y * 3 + xi * 3 + 1] > 200) && (data[yi*y * 3 + xi * 3 + 2] < 100) ? true : false;
	...
	mFlats.insert(mFlats.begin(), 100, GetEnvironmentBuilder()->getCube());
	mFlats.clear();
	for (int xi = 0; xi < MAP_WIDTH; ++xi)
	{
		for (int yi = 0; yi < MAP_WIDTH; ++yi)
		{
			...
			if (d.destructable)
			{
			  ...
			  temp->Initialise(*mMeshMg->GetMesh("cube"), false);

			  temp->GetPosition() = Vector3(xi*wscale + SizeOffset, 0.02f, yi*wscale + SizeOffset);
			  temp->GetScale() = Vector3(0.045f, 0.045f, 0.045f);


			  MaterialExt mat = temp->GetMesh().GetSubMesh(0).material;
			  mat.gfxData.Set(Vector4(0.9f, 0.8f, 0.8f, 0), Vector4(0.9f, 0.8f, 0.8f, 0), Vector4(0.9f, 0.8f, 0.8f, 1));
			  mat.pTextureRV = mFX->mCache.LoadTexture("data/floor.dds", false, gd3dDevice);
