...
{
	assert(id >= 0 && id < 8);
	gGfxPerFrame.lights[id].type = Light::Type::OFF;
}
...
{
	File f(fileName, File::MPF_READ);
	unsigned int toRead = f.getSize();
	char* pBuff = new char[toRead];
	f.read(pBuff, toRead, bytesRead);
...
bool MyFX::Init(ID3D11Device* pd3dDevice)
{
	...
	char* pBuff = ReadAndAllocate("data/TextureVS.cso", bytes);
	...
	mpSpriteB = new SpriteBatch(gd3dImmediateContext);
	...
	pBuff = ReadAndAllocate("data/VSLine.cso", bytes);
	CreateVertexShader(pBuff, bytes, mpVSLine);
	CreateInputLayout(VertexPosColour::sVertexDesc, 2, pBuff, bytes, &mpInputLayoutLine);
	delete[] pBuff;
	pBuff = ReadAndAllocate("data/PSLine.cso", bytes);
...
void MyFX::FlushLines(ID3D11DeviceContext *pD3DContext)
{
	...
	FX::GetProjectionMatrix() = Matrix::CreateOrthographic(sw, sh, 1, 1000);
...
void MyFX::RenderLine(const Vector2& start, const Vector4& startCol, const Vector2& end, const Vector4& endCol, float z)
{
	assert(mLineIdx < MAX_LINES);
	VertexPosColour& v = mLineVB[mLineIdx * 2];
	v.Pos = Vector3(start.x, start.y, z);
	v.Colour = startCol;
	VertexPosColour& v2 = mLineVB[mLineIdx * 2 + 1];
	v2.Pos = Vector3(end.x, end.y, z);
	v2.Colour = endCol;
...
void MyFX::Render(Model& model, ID3D11DeviceContext *pD3DContext, MaterialExt* pOverrideMat, Matrix* pOverrideMtx)
{
	//setup shaders
