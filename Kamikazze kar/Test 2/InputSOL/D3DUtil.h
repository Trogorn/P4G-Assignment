...
radius = raduius of sphere
ray = the ray
dist = distance down the ray to entry point
NOTE - this is built into DirectXTK ray, but you need to know how it works. Once happy with this code,
have a look at the SimpleMath version in Ray, it looks more complicated because it needs to use SIMD registers
for speed on XboxOne, hence we have this simpler version to compare it to.
*/
bool RayToSphere(const DirectX::SimpleMath::Vector3& centre, float radius, const DirectX::SimpleMath::Ray& ray, float& dist);
/*
Say if a ray collides with a plane
point - a 3D coordinate on the plane
normal - a plane normal, which way the plane faces
ray - the ray
dist - distance down the ray to the collision with the plane
*/
