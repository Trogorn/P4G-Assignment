...
void Game::Initialise()
{
	...
	*mCube->GetPosition() = Vector3(0.0f, 0.5f, 0.5f);
	...
	mRock = new Model_Kami();
	mRock->Initialise(ms);
	*mRock->GetPosition() = Vector3(0, 0, 0);
	*mRock->GetRotation() = Vector3(0, 0, 0);
	*mRock->GetScale() = Vector3(0.1f, 0.1f, 0.1f);
...
void Game::Render(float dTime)
{
	...
	*mRock->GetPosition() = mCamera.GetPos() + Vector3(0, 0, 2);
	...
	//mpSpriteBatch->Draw(mFX.mCache.LoadTexture("test.dds", true, gd3dDevice), Vector2(sw / 2, sh / 4), nullptr, Colours::White, 0, Vector2(0, 0), Vector2(0.01f, 0.01f)); TODO Fix this weird bug, path seems to still be directed at Rock, do we have to reset it?
	wstringstream ss;
	if (dTime > 0)
		ss << L"FPS: " << (int)(1.f / dTime);
	else
