...
DirectX::SimpleMath::Vector2 MouseAndKeys::GetMouseMoveAndCentre()
{
	POINT pt;
	Vector2 m(0, 0);
	if (GetCursorPos(&pt))
	{
		m = Vector2((float)pt.x - mCentredMouseMove.x, (float)pt.y - mCentredMouseMove.y);
		mCentredMouseMove = CentreMouse();
		//DBOUT("Mouse x=" << pt.x << " y=" << pt.y << ", adj x=" << m.x << " y=" << m.y << " centred x=" << mCentredMouseMove.x << " y=" << mCentredMouseMove.y);
	}
	return m;
}

Vector2 MouseAndKeys::CentreMouse(bool readOnly)
{
	int w, h;
	GetClientExtents(w, h);
	POINT pt;
	...
	GetCursorPos(&pt); //it doesn't always exactly set where you want so ask it for the exact position
	//DBOUT("Mouse reset x=" << pt.x << " y=" << pt.y);
