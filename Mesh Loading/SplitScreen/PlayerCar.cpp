...
void PlayerCar::Initialise(Model* mModel, float turnSpeed, float drag, float acceleration, float brakingForce, float maxSpeed, float reverseSpeed, Turret* turret)
{
	this->turret = turret;
...
void PlayerCar::UpdateCamera(float dTime)
{
	...
	*camera.GetCameraPosition() = Vector3::Transform(cameraPosL, L2W);
	...
	turret->SetPosition(Vector3(0, 0.1f, 0) + *GetPosition());
}
